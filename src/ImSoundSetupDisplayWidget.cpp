...
namespace ImSound {
SetupDisplayWidget::SetupDisplayWidget() {
  setClientName("ImSound::SetupDisplayWidget");
}

void SetupDisplayWidget::paint() {
  if (_setup && ImGui::BeginTable("Setup Table", 2)) {
    ...
    appendRow();
    appendRow("NONINTERLEAVED",
              _setup->checkStreamFlags(RTAUDIO_NONINTERLEAVED) ? "True"
                                                               : "False");
...
void SetupDisplayWidget::applyStreamConfig(const RtSound::StreamSetup &setup) {
  _setup = std::make_unique<RtSound::StreamSetup>(setup);
}
